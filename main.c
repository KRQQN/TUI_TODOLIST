...
int main() {
  ...
  WINDOW *win =
      newwin(task_box_height, task_box_width, (scr_rows - task_box_height) / 2,
             (scr_cols - task_box_width) / 2);
  ...
  while (is_running) {
    ...
    case 'a':
      Task new_task;
      WINDOW *add_task_win =
          newwin(3, task_box_width, (scr_rows + task_box_height) / 2,
                 (scr_cols - task_box_width) / 2);
...
void render_tasks(WINDOW *window, Task tasks[], int task_count,
                  int highlighted_task) {
  werase(window);
  box(window, 0, 0);
  for (int i = 0; i < task_count; i++) {
    if (i == highlighted_task) {
      wattron(window, COLOR_PAIR(1));
      mvwprintw(window, i + 1, 2, "* %s [%s]", tasks[i].text,
                tasks[i].completed ? "x" : " ");
